	...
	if #card > 1 then
		while card[i] ~= nil and i < 17 do
			if card[i] == card[i-1] then
				-- statu = statu | (3 << (i-1)*2)
				statu = bit.bor(statu, bit.lshift(3, (i-1)*2))
				temp_count = temp_count +1
			elseif card[i] == (card[i-1] + 1) then
				-- print(string.format("card[%d] = [%d], card[i-1] = %d", i, card[i], card[i-1]))
				-- statu = statu | (2 << (i-1)*2)
				statu = bit.bor(statu, bit.lshift(2, (i-1)*2))
				temp_count = 1
			else
				-- statu = statu | (1 << (i-1)*2)
				statu = bit.bor(statu, bit.lshift(1, (i-1)*2))
				temp_count = 1
			end
	    ...
	    if i == 17 then 
	        while i < 21 and card[i] and card[i] ~= -1 and card[i] ~= 0 do 
	            if card[i] == card[i-1]  then 
	                ...
	                --more_statu = bit.bor(more_statu, bit.lshift(3, (i+1 - 17)*2))
					more_statu = bit:bor(statu, bit:lshift(3, (i+1 - 17)*2))
	                temp_count = temp_count + 1
	            elseif card[i] + 1 == card[i-1] then 
	                -- statu = statu | (2 << (i+1 - 17)*2)
	                more_statu = bit.bor(more_statu, bit.lshift(2, (i+1 - 17)*2))
	                temp_count = 1
	            else 
	            	-- statu = statu | (1 << (i+1 - 17)*2)
	                more_statu = bit.bor(more_statu, bit.lshift(1, (i+1 - 17)*2))
	                temp_count = 1
	            end 
...
function JudgeCard:MoreCard(arg_low, arg_high, arg_value)
    ...
    elseif arg_high == 340 then 
    	-- print("arg_high == 340")
    	-- print("arg_low", arg_low, arg_low == -1090785347)
