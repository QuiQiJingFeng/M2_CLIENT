--扭蛋机界面
local LimiteEggRewardLayer = class("LimiteEggRewardLayer", function()
    return display.newColorLayer(cc.c4b(0, 0, 0, 255))
end)

LimiteEggRewardLayer._winScale = math.min(lt.CacheManager:getWinScale(), display.size.width / display.size.height *  640 / 1136) 

--LimiteEggRewardLayer._winScale = display.size.width / display.size.height *  640 / 1136
--LimiteEggRewardLayer._winScale =  math.min(display.width / lt.Constants.BGWIDTH, display.height / lt.Constants.BGHEIGHT)

LimiteEggRewardLayer._bg = nil
LimiteEggRewardLayer._panelSize = cc.size(363,280)

LimiteEggRewardLayer._callback = nil
LimiteEggRewardLayer._delegate = nil
...
local AUTO_TIME = 11
function LimiteEggRewardLayer:ctor(delegate, params)--params 用于其他界面跳转到扭蛋机界面 数据改变切回去的数据刷新
    ...
    --标题背景
    self._bg = display.newSprite("image/ui/egg_limite_bg.jpg")
    ...
    self._diamondPoolLabel:setPosition(self._bg:getContentSize().width - 270,self._bg:getContentSize().height - 35)
    ...
    --奖励列表
    local listBgSize = cc.size(295, 500)

    self._eggListView = lt.ListView.new({
        viewRect  = cc.rect(0, 2, 295* self._winScale, 495 * self._winScale),
    ...
    self._contentNode:setContentSize(295*self._winScale, 500 * self._winScale)
    ...
    self._contentNode:setPosition(worldPos.x - 135 * self._winScale * self._winScale, worldPos.y + 36 * self._winScale)
    ...
    --特效背景
    local effectBgName = "ui_effect_thomas_zise"
    ...
    self._effectBg1:setPosition(self._bg:getContentSize().width / 2 - 4, self._bg:getContentSize().height / 2)
    self._bg:addChild(self._effectBg1)     

    self._effectBg2 = ccs.Armature:create(effectBgName)--背景 未点击时的常驻特效 1  点击时 2
    self._effectBg2:setPosition(self._bg:getContentSize().width / 2 - 4, self._bg:getContentSize().height / 2)
    ...
    self._effectBg3:setPosition(self._bg:getContentSize().width / 2, self._bg:getContentSize().height / 2)
...
function LimiteEggRewardLayer:updateTime(maxTime)
...
function LimiteEggRewardLayer:onUpdate(delta)
...
function LimiteEggRewardLayer:clearTipsItem()
...
function LimiteEggRewardLayer:clickEggItem(event)
...
function LimiteEggRewardLayer:updateEggList(refresh, refreshType)
    ...
    for row = 1, #newItemList + 1 do
        ...
        item:setItemSize(280 *self._winScale, 110 * self._winScale + (self._winScale - 1) * 25)
...
function LimiteEggRewardLayer:eggStart()--点击旋转按钮
...
function LimiteEggRewardLayer:onEggStart(event)--扭蛋数据
...
function LimiteEggRewardLayer:chatSystem()--获得的物品在系统广播
...
function LimiteEggRewardLayer:eggEffectPlay()--扭蛋的特效处理
    ...
    if self.is_color_egg  == 0 then --0是红蛋，1是彩蛋
        effectName = "ui_effect_thomas_purple_egg"
    else
    ...
    if self.is_color_egg  == 0 then
        ...
        colorEgg_1:setPosition(display.cx + 15, display.cy)
...
function LimiteEggRewardLayer:eggOpenClick(event)--彩蛋的点击处理
...
function LimiteEggRewardLayer:onEggGetReward(event)--彩蛋的奖励信息
...
function LimiteEggRewardLayer:openColorEgge( )
    local tag = 1
...
function LimiteEggRewardLayer:onMaskTouch(event)
    --event.targe
...
function LimiteEggRewardLayer:onEggRefresh(event)
    self:updateEggList(false, EGG_POOL_REFRESH_TYPE.CLICK_BTN_REFRESH)
end

function LimiteEggRewardLayer:refreshEggPool()--如果中大奖保存大奖的数量，当玩家关闭开蛋界面在用最新值,待扭蛋数据赋值之后再刷
    self:runAction(
        cca.seq{
            cca.delay(0.1),
            cca.cb(function ( )
...
function LimiteEggRewardLayer:refreshBgEffect()
    local currentItemCount = lt.DataManager:getItemCount(lt.Constants.ITEM.EGG_COIN)
    if currentItemCount <= 0 then
        self._effectBg3:getAnimation():playWithIndex(6)
    else
...
function LimiteEggRewardLayer:refreshEggCoinAdd()
    local currentItemCount = lt.DataManager:getItemCount(lt.Constants.ITEM.EGG_COIN)
...
function LimiteEggRewardLayer:onShopClubItemRefresh()
    self:onRefreshItem()
end

function LimiteEggRewardLayer:onRefreshItem()--通用的获取途径中购买回调刷新
    self:refreshBgEffect()
    self:refreshEggCoinAdd()
end

function LimiteEggRewardLayer:onClickEggPool()--点击奖池查看获奖名单
    lt.SocketApi:getEggRewardLogList()
end

function LimiteEggRewardLayer:onEggRewardLogList(event)--最近获得最是大奖的名单
...
function LimiteEggRewardLayer:eggRefresh()
    local eggArray = lt.DataManager:getEggArray()
    -- local eggInfoTable = lt.CacheManager:getEggRewardTable()
...
function LimiteEggRewardLayer:onPoolPanelTouch()--
    self._rewardPoolPanel:setVisible(false)
end

function LimiteEggRewardLayer:onClose()
    if self._delegate then
        self._delegate:clearLimiteEggRewardLayer()
    end
end

function LimiteEggRewardLayer:onEnter()
    ...
    lt.SocketApi:addListener(lt.SocketConstants.ID_S2C_EGG_START, handler(self, self.onEggStart), "LimiteEggRewardLayer:onEggStart")
    lt.SocketApi:addListener(lt.SocketConstants.ID_S2C_EGG_REFRESH, handler(self, self.onEggRefresh), "LimiteEggRewardLayer:onEggRefresh")
    lt.SocketApi:addListener(lt.SocketConstants.ID_S2C_GET_EGG_REWARD_LOG_LIST, handler(self, self.onEggRewardLogList), "LimiteEggRewardLayer:onEggRewardLogList")
    lt.GameEventManager:addListener(lt.GameEventManager.EVENT.PLAYER_EGG_POOL_REFRESH, handler(self, self.refreshEggPool), "LimiteEggRewardLayer:refreshEggPool")
    lt.SocketApi:addListener(lt.SocketConstants.ID_S2C_EGG_GET_REWARD, handler(self, self.onEggGetReward), "LimiteEggRewardLayer:onEggGetReward")
    lt.GameEventManager:addListener(lt.GameEventManager.EVENT.SHOP_CLUB_ITEM_REFRESH, handler(self, self.onShopClubItemRefresh), "LimiteEggRewardLayer:onShopClubItemRefresh")
...
function LimiteEggRewardLayer:onExit()
    lt.SocketApi:removeEventListenersByTag("LimiteEggRewardLayer:onEggStart")
    lt.SocketApi:removeEventListenersByTag("LimiteEggRewardLayer:onEggRefresh")
    lt.SocketApi:removeEventListenersByTag("LimiteEggRewardLayer:onEggRewardLogList")
    lt.GameEventManager:removeListener("LimiteEggRewardLayer:refreshEggPool")
    lt.SocketApi:removeEventListenersByTag("LimiteEggRewardLayer:onEggGetReward")
    lt.GameEventManager:removeListener("LimiteEggRewardLayer:onShopClubItemRefresh")
    -- 移除图片
